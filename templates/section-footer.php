...
if (!class_exists('Integrio_footer_area')) {
	/**
	 * Footer area
	 *
	 *
	 * @class 		Integrio_footer_area
	 * @version		1.0
	 * @category	Class
	 * @author 		WebGeniusLab
	 */
    ...
    class Integrio_footer_area {
		...
		*/  	
    	...
    	function __construct () {
	        ...
	        $footer_bg_color = Integrio_Theme_Helper::get_option('footer_bg_color');
	        // copyright option
	        $copyright_switch = Integrio_Theme_Helper::get_option('copyright_switch');
			
			//add global variables
	        $this->footer_full_width = Integrio_Theme_Helper::get_option('footer_full_width');
	        ...
	        $style .= Integrio_Theme_Helper::bg_render('footer','mb_footer_switch','on');
	        ...
	        */
	        if ($footer_switch || $copyright_switch) {
	            echo "<footer class='footer clearfix'".$style." id='footer'>";
	                if ($footer_switch) {
	                	$footer_content_type = Integrio_Theme_Helper::options_compare('footer_content_type','mb_footer_switch','on');
    	...
    	private function get_wave_html(){
    		$wave_switch = Integrio_Theme_Helper::options_compare('footer_add_wave','mb_footer_switch','on');
    		if((bool) $wave_switch){

    			$wave_height = Integrio_Theme_Helper::get_option('footer_wave_height');
    			...
    			echo "<div class='integrio_wave_footer' style='height:".(int) $wave_height."px;'>";
    	...
    	private function get_footer_vars($optn_1 = null){
    		...
    		//Get options	
			$footer_spacing = Integrio_Theme_Helper::options_compare('footer_spacing','mb_footer_switch','on');

	        // Only for widgets in footer
	        if ($optn_1 == 'widgets') {
				$footer_options['widget_columns'] = Integrio_Theme_Helper::get_option('widget_columns');
		        $footer_options['widget_columns_2'] = Integrio_Theme_Helper::get_option('widget_columns_2');
		        $footer_options['widget_columns_3'] = Integrio_Theme_Helper::get_option('widget_columns_3');
		        $footer_align = Integrio_Theme_Helper::get_option('footer_align');
    	...
    	private function main_footer_get_page(){
	        ...
	        echo "<div class='footer_top-area'>";
                ...
                echo "<div class='row-footer'".$footer_style.">";
                    ...
                    $footer_page_select = Integrio_Theme_Helper::options_compare('footer_page_select','mb_footer_switch','on');
    	...
    	private function copyright_spacing(){
	        //Get options
    		$copyright_spacing = Integrio_Theme_Helper::options_compare('copyright_spacing','mb_copyright_switch','on');
    	...
    	private function copyright_style(){
			$bg_color = Integrio_Theme_Helper::options_compare('copyright_bg_color','mb_copyright_switch','on');
    	...
    	private function copyright_html() {	
	        $editor = Integrio_Theme_Helper::get_option('copyright_editor');

	        if ($this->mb_copyright_switch == 'on') {
	        	$editor = rwmb_meta('mb_copyright_editor');
	        }
	        ?>
    		<div class='copyright'<?php echo Integrio_Theme_Helper::render_html($this->copyright_style()); ?> >
                <?php if (!$this->footer_full_width) echo "<div class='wgl-container'>"; ?>
                	<div class='row' <?php echo Integrio_Theme_Helper::render_html($this->copyright_spacing());?> >
