...
if (!class_exists('Integrio_get_header')) {
	class Integrio_get_header{
	    ...
	    public function header_vars(){
	    	...
	    	$this->name_preset = Integrio_Theme_Helper::get_option('header_def_js_preset');
	    	$get_def_name = get_option( 'integrio_set_preset' );
	    	if( !$this->in_array_r($this->name_preset, get_option( 'integrio_set_preset' ))){
	    	...
	    	if (class_exists( 'RWMB_Loader' ) && $this->id !== 0) {
	    		...
	    		if (!empty($customize_header) && $customize_header != 'default') {
	    			if( !$this->in_array_r(rwmb_meta('mb_customize_header'), get_option( 'integrio_set_preset' ))){
	    			}else{
	    				$get_def_name = get_option( 'integrio_set_preset' );
		...
		*/
	   	public function header_class(){
	   		$header_shadow = Integrio_Theme_Helper::get_option( 'header_shadow',$this->name_preset, $this->def_preset );
	   		$header_on_bg = Integrio_Theme_Helper::get_option( 'header_on_bg',$this->name_preset, $this->def_preset );
		...
		*/
		...
		public function header_bar_editor($location = null,$position = null){
			...
			*/
			...
			${'header_'.$position.'_editor'} = Integrio_Theme_Helper::get_option($location.'_header_bar_'.$position.'_editor', $this->name_preset, $this->def_preset);
		...
		*/
		...
		public function header_bar_delimiter($k = null){
			...
			*/
			...
			$height = Integrio_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_height', $this->name_preset, $this->def_preset );
			$width = Integrio_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_width', $this->name_preset, $this->def_preset );

			$bg_color = Integrio_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_bg', $this->name_preset, $this->def_preset );

			$margin = Integrio_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_margin', $this->name_preset, $this->def_preset );
	        ...
	        if ( $this->html_render === 'sticky' ) {
	        	$custom_sticky = Integrio_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_custom', $this->name_preset, $this->def_preset);
	        	if( !empty($custom_sticky) ){
	        		$bg_color = Integrio_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_color', $this->name_preset, $this->def_preset);
	        	}
	        }
		...
		*/
		...
		public function header_bar_button($k = null){
			...
			*/
			...
			$get_number = (int) filter_var($k, FILTER_SANITIZE_NUMBER_INT);
			$button_text = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_title', $this->name_preset, $this->def_preset);

			$link = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_link', $this->name_preset, $this->def_preset);

			$target = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_target', $this->name_preset, $this->def_preset);
			$target = !empty($target) ? 'target:%20_blank' : '';

			$size = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_size', $this->name_preset, $this->def_preset);

			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_custom'.$options_btn, $this->name_preset, $this->def_preset);

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_bg'.$options_btn, $this->name_preset, $this->def_preset);

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_color_txt'.$options_btn, $this->name_preset, $this->def_preset);

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_border'.$options_btn, $this->name_preset, $this->def_preset);
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_bg'.$options_btn, $this->name_preset, $this->def_preset);
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_color_txt'.$options_btn, $this->name_preset, $this->def_preset);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = Integrio_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_border'.$options_btn, $this->name_preset, $this->def_preset);
		...
		*/
		...
		public function header_bar_spacer($location = null, $key = null){
			...
			*/
			...
			$spacer = Integrio_Theme_Helper::get_option($location.'_header_spacer'.$get_number, $this->name_preset, $this->def_preset);
		...
		*/
	   	public function build_header_layout($section = 'bottom'){
	   		...
	   		$header_sticky_builder = Integrio_Theme_Helper::get_option('sticky_header');
	   		...
	   		$header_layout = Integrio_Theme_Helper::get_option($section.'_header_layout', $this->name_preset, $this->def_preset);
	   		$lavalamp_active = Integrio_Theme_Helper::get_option('lavalamp_active', $this->name_preset, $this->def_preset);
						    					...
						    					if ($key != 'placebo' && $key != 'pos_column') {
						    						switch ($key) {
														...
														if($this->html_render === 'mobile'){
															$menu = Integrio_Theme_Helper::get_option('mobile_menu');
														}
			...
			*/
			...
			switch ($section) {
				case 'bottom':
					$width_container = Integrio_Theme_Helper::get_option('header'.$s.'_full_width',$this->name_preset, $this->def_preset);
				...
				case 'sticky':
					$width_container = Integrio_Theme_Helper::get_option('header_custom_sticky_full_width');
					if ($width_container == '1') {
			   			$class = "fullwidth-wrapper";
			   		} else {
			   			$class = 'wgl-container';
			...
			* @access private
			*/
			$header_background = Integrio_Theme_Helper::get_option('header'.$s.'_background', $this->name_preset, $this->def_preset);
			$header_background_image = Integrio_Theme_Helper::get_option('header'.$s.'_background_image', $this->name_preset, $this->def_preset);
			$header_background_image = isset($header_background_image['url']) ? $header_background_image['url'] : '';

			$header_color = Integrio_Theme_Helper::get_option('header'.$s.'_color', $this->name_preset, $this->def_preset);
			$header_bottom_border = Integrio_Theme_Helper::get_option('header'.$s.'_bottom_border', $this->name_preset, $this->def_preset);
			$header_border_height = Integrio_Theme_Helper::get_option('header'.$s.'_border_height', $this->name_preset, $this->def_preset);
			$header_border_height = $header_border_height['height'] ?? '';
			$header_bottom_border_color = Integrio_Theme_Helper::get_option('header'.$s.'_bottom_border_color', $this->name_preset, $this->def_preset);
			...
			*/
			...
			$header_sticky_height = Integrio_Theme_Helper::get_option('header_sticky_height');
	   		...
	   		$header_height = Integrio_Theme_Helper::get_option('header'.$s.'_height', $this->name_preset, $this->def_preset);
			...
			* @access private
			*/
			$dispay = Integrio_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_display', $this->name_preset, $this->def_preset);
			$v_align = Integrio_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_vert', $this->name_preset, $this->def_preset);
			$h_align = Integrio_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_horz', $this->name_preset, $this->def_preset);
		...
		*/
	   	public function build_header_mobile_menu($preset = null, $def_preset = null){
	   		...
	   		$header_queris = Integrio_Theme_Helper::get_option('header_mobile_queris' ,$preset, $def_preset);
	    ...
	    public function header_render_html(){
	    	$mobile_header_custom =  Integrio_Theme_Helper::get_option('mobile_header');
		...
		*/
        public static function get_logo($location){
            ...
            $header_logo_src = Integrio_Theme_Helper::get_option('header_logo');
            ...
            // Get Sticky Logotype
            $logo_sticky_src =  Integrio_Theme_Helper::get_option('logo_sticky');
            ...
            // Get Mobile Logotype
            $logo_mobile_src =  Integrio_Theme_Helper::get_option('logo_mobile');
            ...
            $logo_height_custom = Integrio_Theme_Helper::get_option('logo_height_custom');
            $logo_height = Integrio_Theme_Helper::get_option('logo_height');
            $logo_height = $logo_height['height'] ?? '';


            $sticky_logo_height_custom = Integrio_Theme_Helper::get_option('sticky_logo_height_custom');
            $sticky_logo_height = Integrio_Theme_Helper::get_option('sticky_logo_height');
            $sticky_logo_height = $sticky_logo_height['height'] ?? '';

            $mobile_logo_height_custom = Integrio_Theme_Helper::get_option('mobile_logo_height_custom');
            $mobile_logo_height = Integrio_Theme_Helper::get_option('mobile_logo_height');
			...
			switch (true) {
				case $location == 'bottom':
					if (!empty($header_logo_src)) {
						?>
						<img class="default_logo" src="<?php echo esc_url($header_logo_src); ?>" alt="<?php echo esc_attr($def_img_alt); ?>" <?php echo Integrio_Theme_Helper::render_html($logo_height_style);?>>
					<?php
					} else {
						?>
						<h1 class="logo-name">
				...
				case !empty($logo_sticky_src) && $location == 'sticky':
					?>
					<img class="logo-sticky" src="<?php echo esc_url($logo_sticky_src);?>" alt="<?php echo esc_attr($sticky_img_alt); ?>" <?php echo Integrio_Theme_Helper::render_html($sticky_height_style);?>>
					<?php
					break;

				case !empty($logo_mobile_src) && $location == 'mobile':
					?>
					<img class="logo-mobile" src="<?php echo esc_url($logo_mobile_src);?>" alt="<?php echo esc_attr($mobile_img_alt); ?>" <?php echo Integrio_Theme_Helper::render_html($mobile_height_style);?>>
					<?php
					break;
		...
		*/
	    public function search($html_render = '',$location = ''){
	    	$description = esc_html__('Type To Search', 'integrio');
			$search_style = Integrio_Theme_Helper::get_option('search_style');
	        ...
	        $search_class = ' search_'.Integrio_Theme_Helper::get_option('search_style');
					...
					$output .= '<div class="header_search-field">';
						if ($search_style === 'alt') {
							$output .= '<div class="header_search-wrap">';
								$output .= '<div class="integrio_module_double_headings aleft">';
								$output .= '<h3 class="header_search-heading_description heading_title">'.apply_filters('integrio_desc_search', $description) .'</h3>';
								$output .= '</div>';
							$output .= '</div>';
	    ...
	    public function login_in($location, $section){
	        ...
	        echo "<div class='login-in woocommerce'".$this->row_style_height($location).">";
	        	...
	        	if ( is_user_logged_in() ){
	        		...
	        		echo "<a class='login-in_link-logout' href='".esc_url( $link_logout )."'>".esc_html__('Logout', 'integrio')."</a>";
	        	}else{
	        		echo "<a class='login-in_link' href='".esc_url_raw( $url )."'>".esc_html__('Login', 'integrio')."</a>";
	        	}
		...
		*/
		...
		public static function icon_cart() {
			...
			?>
				<a class="woo_icon" title="<?php esc_attr_e( 'Click to open Shopping Cart', 'integrio' ); ?>">
