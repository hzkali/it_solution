...
/**
* Integrio Woocommerce
*
*
* @class        Integrio_Woocoommerce
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/

if (!class_exists('Integrio_Woocoommerce')) {
	class Integrio_Woocoommerce{
	    /**
		...
		*/
		...
		public function setup() {
			...
			add_theme_support( 'woocommerce', apply_filters( 'integrio_woocommerce_args', array(
				'single_image_width'    => 1080,
				'thumbnail_image_width' => 540,
				'gallery_thumbnail_image_width' => 240,
				'product_grid'          => array(
					'default_columns' => (int) Integrio_Theme_Helper::get_option('shop_column'),
					'default_rows'    => 4,
					'min_columns'     => 1,
					'max_columns'     => 6,
					'min_rows'        => 1,),
		...
		public function init (){
			...
			add_action( 'woocommerce_shop_loop_subcategory_title', array( $this, 'wgl_template_loop_category_title' ), 10 );
		...
		function review_comments_meta_info($comment){
			...
			if ( '0' === $comment->comment_approved ) { ?>
				<em class="woocommerce-review__awaiting-approval">
					<?php esc_html_e( 'Your review is awaiting approval', 'integrio' ); ?>
				</em>
			...
			<?php } else { ?>
				...
				if ( 'yes' === get_option( 'woocommerce_review_rating_verification_label' ) && $verified ) {
					echo '<em class="woocommerce-review__verified verified">(' . esc_attr__( 'verified owner', 'integrio' ) . ')</em> ';
				}
				?>
		...
		function wgl_filter_comments($comment_form){
			...
			$comment_form = array(
				'title_reply'          => have_comments() ? esc_html__( 'Add a review', 'integrio' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'integrio' ), get_the_title() ),
				'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'integrio' ),
				'title_reply_before'   => '<span id="reply-title" class="comment-reply-title">',
				'title_reply_after'    => '</span>',
				'comment_notes_after'  => '',
				'fields'               => array(
					'author' => '<p class="comment-form-author">' . '<label for="author"></label> ' .
					'<input id="author" name="author" placeholder="'.esc_attr__( 'Name', 'integrio' ).'" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" aria-required="true" required /></p>',
					'email'  => '<p class="comment-form-email"><label for="email"></label> ' .
					'<input id="email" name="email" placeholder="'. esc_attr__( 'Email', 'integrio' ).'" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" aria-required="true" required /></p>',
				),
				'label_submit'  => esc_html__( 'Submit', 'integrio' ),
				'logged_in_as'  => '',
				'comment_field' => '',
			);
			...
			if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
				...
				$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( wp_kses( __( 'You must be <a href="%s">logged in</a> to post a review.', 'integrio' ), $allowed_html), esc_url( $account_page_url ) ) . '</p>';
			}

			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
				$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'integrio' ) . '</label><select name="rating" id="rating" aria-required="true" required>
				<option value="">' . esc_html__( 'Rate&hellip;', 'integrio' ) . '</option>
				<option value="5">' . esc_html__( 'Perfect', 'integrio' ) . '</option>
				<option value="4">' . esc_html__( 'Good', 'integrio' ) . '</option>
				<option value="3">' . esc_html__( 'Average', 'integrio' ) . '</option>
				<option value="2">' . esc_html__( 'Not that bad', 'integrio' ) . '</option>
				<option value="1">' . esc_html__( 'Very poor', 'integrio' ) . '</option>
				</select></div>';
			}

			$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment"></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="'.esc_attr__( 'Your review', 'integrio' ).'" required></textarea></p>';
			return $comment_form;
		}
		...
		public function loop_products_per_page() {
			return (int) Integrio_Theme_Helper::get_option('shop_products_per_page');
		}
		/**/
		/* \LOOP */
		/**/
		...
		public function related_products_args( $args ) {
			$args = array(
				'posts_per_page' => (int) Integrio_Theme_Helper::get_option('shop_r_products_per_page'),
				'columns'        => (int) Integrio_Theme_Helper::get_option('shop_related_columns'),
			);
		...
		public function loop_columns( $args ) {
			$columns = (int) Integrio_Theme_Helper::get_option('shop_column'); // 3 products per row
			return $columns;
		}

		public function wgl_product_columns_wrapper() {
			$columns = (int) Integrio_Theme_Helper::get_option('shop_column');
		...
		public function init_template(){
			...
			$sb = Integrio_Theme_Helper::render_sidebars('shop_'.$shop_template);
        ...
        function wgl_template_loop_category_title( $category ) {
            ?>
            <div class="woo_product_content">
                <h2 class="woocommerce-loop-category__title woocommerce-loop-product__title">
                    <?php
                    echo esc_html( $category->name );
                
                    if ( $category->count > 0 ) {
                        echo apply_filters( 'woocommerce_subcategory_count_html', ' <mark class="post_count count">(' . esc_html( $category->count ) . ')</mark>', $category ); // WPCS: XSS ok.
                    }
                    ?>
                </h2>
            </div>
...
new Integrio_Woocoommerce ( );

// Integrio Woocoommerce Helpers

if ( ! function_exists( 'integrio_woocommerce_breadcrumb' ) ) {
	/**
	 * Output the WooCommerce Breadcrumb.
	 *
	 * @param array $args Arguments.
	 */
	function integrio_woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => esc_html_x( 'Home', 'breadcrumb', 'integrio' ),
		) ) );
		...
		if ( ! empty( $breadcrumb ) ) {
			...
			$out .= Integrio_Theme_Helper::render_html($wrap_before);

			foreach ( $breadcrumb as $key => $crumb ) {

				$out .=  Integrio_Theme_Helper::render_html($before);
				...
				$out .=  Integrio_Theme_Helper::render_html($after);

				if ( sizeof( $breadcrumb ) !== $key + 1 ) {
					$out .=  Integrio_Theme_Helper::render_html($delimiter);
				}
			}
			$out .=  Integrio_Theme_Helper::render_html($wrap_after);
		}
